...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='circles' AND column_name='status') THEN
    ALTER TABLE circles ADD COLUMN status VARCHAR(20) DEFAULT 'active';
  END IF;
END $$;

-- Step 2: Insert data (skips duplicates)
INSERT INTO circles (name, description, icon, members, posts, color, category, status)
SELECT * FROM (VALUES
  ('Anxiety Support', 'Safe space for managing anxiety and stress', '🌊', 1247, 892, '#6366F1', 'mental-health', 'active'),
  ('Depression Corner', 'You''re not alone in this journey', '🌱', 1583, 1204, '#8B5CF6', 'mental-health', 'active'),
  ('Work & Career Stress', 'Navigate workplace challenges together', '💼', 934, 567, '#3B82F6', 'lifestyle', 'active'),
  ('Relationship Talk', 'Love, family, and friendship support', '❤️', 1876, 1345, '#EC4899', 'support', 'active'),
  ('Student Life', 'Academic pressure and campus struggles', '📚', 756, 423, '#F59E0B', 'lifestyle', 'active'),
  ('LGBTQ+ Safe Space', 'Acceptance, understanding, belonging', '🏳️🌈', 1092, 789, '#EF4444', 'support', 'active'),
  ('New Parents', 'Parenting wins and struggles', '👶', 623, 334, '#10B981', 'lifestyle', 'active'),
  ('General Wellness', 'Daily life and self-care', '✨', 1456, 987, '#14B8A6', 'mental-health', 'active')
) AS v(name, description, icon, members, posts, color, category, status)
